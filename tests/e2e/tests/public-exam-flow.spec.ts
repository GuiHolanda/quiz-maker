...
import { E2E_PUBLIC_EXAM_NAME, E2E_SUBJECT } from '../fixtures/mock-data';

// The public exam (E2E_PUBLIC_EXAM_NAME) and its 3 questions are seeded by globalSetup,
// so this flow starts at question generation — no wizard step needed.
test('public exam full flow: questions → simulado → answer → result → retry & cancel', async ({
  authedPage: page,
}) => {
          ...
          setTimeout(() => {
            this.dispatchEvent(new MessageEvent('awaiting_review', {
              data: JSON.stringify({
                doneTopics: 1,
                totalTopics: 1,
                topics: [{ id: 't1', topicName: 'E2E Subject', questionCount: 3, status: 'done', savedCount: 0, errorMessage: null }],
              }),
            }));
          }, 100);
        }
  ...
  await page.route('**/api/generation-job', (route) => {
    if (route.request().method() === 'POST') {
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ jobId: 'e2e-exam-job' }) });
    } else {
  ...
  // The exam is pre-filled by the SIMULADO_NEW_PREFILL_KEY set during save.
  ...
  const answerButton = page.locator('[data-testid="simulado-card"]', { hasText: E2E_PUBLIC_EXAM_NAME })
    .getByRole('button', { name: /Responder|Continuar|Answer/i }).first();
