...
// annex (quadro de vagas, gabarito, resultado, convocação, retificação, ...) using only the
// URL/filename tokens — no network, no LLM. The locate step's LLM+webSearch call cannot do
// this reliably: the most-indexed PDF of a concurso is often an annex, and its filename usually
// still contains the word "edital", so the model rationalizes picking it. We reorder and flag
...
const ANNEX_PATTERNS: readonly RegExp[] = [
  ...
  /\brecursos?\b/,
  /\bretificacao\b/,
  /\bret\d+\b/, // ret1, ret2 — abbreviated retificação suffix
  /\bcronograma\b/,
  /\bisencao\b/,
  /\bhomologacao\b/,
  /\berrata\b/,
...
// percent-encoding, drop accents, and collapse every separator to a space so token patterns
// with \b match uniformly across "_", "-", "%20", ".", etc.
...
export function classifyEditalUrl(url: string): EditalDocumentKind {
  ...
  // Annex tokens dominate a co-occurring "edital": "Quadro Vagas_Edital ...ret1.pdf" scores
  // annex 3 / main 1 and is correctly flagged. A genuine "Edital de Abertura" that merely
