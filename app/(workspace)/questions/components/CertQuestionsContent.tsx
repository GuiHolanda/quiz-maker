...
import type { AIQuestion, FullExamJobTopicStatus } from '@/shared/types';
        ...
        if (selectedCertification) {
          try {
            localStorage.setItem(
              ...
              JSON.stringify({ type: 'certification', certKey: selectedCertification.key, totalQuestions: selectedCertification.totalQuestions }),
            );
          } catch {}
          addNotification({
            title: t('notification.fullExamTitle'),
            description: t('notification.fullExamDescription', { certName: selectedCertification.label, total: data.savedCount, topics: data.doneTopics }),
  ...
  async function handleFormSubmit(numQuestions: string) {
    ...
    try {
      const { jobId } = await createFullExamJob({
        ...
        distribution: [{ topicName: selectedTopic, questionCount: parseInt(numQuestions, 10) || 5 }],
      });
      setBatchJobId(jobId);
      ...
      const pending = job.topics
        ...
        .flatMap((topic) => {
          ...
          return topicWithJson.pendingQuestionsJson
            ? (JSON.parse(topicWithJson.pendingQuestionsJson) as AIQuestion[])
            : [];
        });
      setAIquestions(pending, null);
      setJobStatus('done');
    } catch {
      notify.error(t('toast.error'), t('toast.somethingWrong'));
    }
