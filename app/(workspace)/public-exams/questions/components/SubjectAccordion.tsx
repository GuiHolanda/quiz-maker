...
export function SubjectAccordion({
  ...
  async function handleBulkDelete() {
    ...
    try {
      const results = await Promise.allSettled(ids.map((id) => deletePublicExamBrowseQuestion(id)));
      ...
      if (deletedIds.size > 0) {
        ...
        onQuestionsLoaded(updated.map((q) => q.id));
        onSelectionChange(new Set(Array.from(selectedIds).filter((sid) => !deletedIds.has(sid))));
      }
