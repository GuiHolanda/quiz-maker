...
const MOCK_SESSIONS = [
  ...
  { id: 2, title: 'AWS SAA — Networking', meta: 'Yesterday · 32 min · 40 questions', score: 67, scoreColor: 'text-warning' },
  { id: 3, title: 'Mock Exam — Full Sim', meta: 'May 17 · 130 min · 65 questions', score: 78, scoreColor: 'text-electric' },
  { id: 4, title: 'AWS SAA — Security & IAM', meta: 'May 16 · 28 min · 35 questions', score: 83, scoreColor: 'text-success' },
  { id: 5, title: 'AWS SAA — Compute & EC2', meta: 'May 15 · 40 min · 50 questions', score: 89, scoreColor: 'text-success' },
] as const;
...
const MOCK_DOMAINS = [
  ...
  { name: 'Networking & Content Delivery', detail: 'Domain 3 · 22% exam weight', score: 67, cohort: 60, color: '#f97316' },
  { name: 'Compute & High Availability', detail: 'Domain 4 · 26% exam weight', score: 89, cohort: 68, color: '#00d4ff' },
  { name: 'Cost Optimization & Billing', detail: 'Domain 5 · 12% exam weight', score: 93, cohort: 70, color: '#00d4ff' },
] as const;
    ...
    <div className="min-h-full bg-default-100/50 px-6 py-6 space-y-5">
      <div className="max-w-7xl mx-auto space-y-5">
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-1 p-6 lg:p-8">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-mono text-[9px] text-default-400 tracking-widest uppercase">
                  AWS SAA-C03
                </span>
              </div>
              <h1 className="font-bold text-foreground text-2xl lg:text-3xl mb-2 leading-snug">
                {greeting}{' '}
                <span className="text-primary">{t('dashboard.onTrack')}</span>
              </h1>
            ...
            <div className="shrink-0 lg:w-64 bg-navy-900 p-6 flex flex-col items-center justify-center relative overflow-hidden">
              ...
              <div className="relative z-10">
                <svg className="overflow-visible" height="100" viewBox="0 0 160 100" width="160">
                  ...
                  <path d="M 20 90 A 60 60 0 0 1 140 90" fill="none" stroke="rgba(59,111,160,0.15)" strokeLinecap="round" strokeWidth="10" />
                  <path d="M 20 90 A 60 60 0 0 1 140 90" fill="none" stroke="url(#gaugeGrad)" strokeDasharray="188.4" strokeDashoffset="41.4" strokeLinecap="round" strokeWidth="10" />
                  <line opacity="0.6" stroke="#4ade80" strokeDasharray="2,2" strokeWidth="1.5" x1="84" x2="84" y1="32" y2="22" />
                  <text fill="#4ade80" fontFamily="monospace" fontSize="8" opacity="0.7" textAnchor="middle" x="84" y="18">Pass</text>
                </svg>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {renderKpi(
            <div className="w-8 h-8 border border-accent/20 rounded-lg flex items-center justify-center" style={{ background: 'rgba(0,212,255,0.07)' }}>
              <FontAwesomeIcon className="text-accent text-xs" icon={faBullseye} />
            </div>,
            <span className="font-mono text-[9px] text-success border border-success/20 rounded px-1.5 py-0.5" style={{ background: 'rgba(74,222,128,0.07)' }}>↑ 3.2%</span>,
            '87.4%',
            t('dashboard.successRate'),
            t('dashboard.successRateDetail', { count: '412' }),
          )}
          {renderKpi(
            <div className="w-8 h-8 border border-orange-400/20 rounded-lg flex items-center justify-center" style={{ background: 'rgba(251,146,60,0.07)' }}>
              <FontAwesomeIcon className="text-orange-400 text-xs" icon={faFire} />
            </div>,
            <span className="font-mono text-[9px] text-orange-400 border border-orange-400/20 rounded px-1.5 py-0.5" style={{ background: 'rgba(251,146,60,0.07)' }}>Personal Best</span>,
            '14 dias',
            t('dashboard.studyStreak'),
            t('dashboard.studyStreakBest', { best: '14', avg: '8.3' }),
          )}
          {renderKpi(
            <div className="w-8 h-8 border border-electric/20 rounded-lg flex items-center justify-center" style={{ background: 'rgba(79,195,247,0.07)' }}>
              <FontAwesomeIcon className="text-electric text-xs" icon={faClock} />
            </div>,
            <span className="font-mono text-[9px] text-default-400 border border-default-200 rounded px-1.5 py-0.5">This week</span>,
            '18h 42m',
            t('dashboard.studyTime'),
            t('dashboard.studyTimeAvg', { avg: '2h 40m' }),
          )}
          {renderKpi(
            <div className="w-8 h-8 border border-success/20 rounded-lg flex items-center justify-center" style={{ background: 'rgba(74,222,128,0.07)' }}>
              <FontAwesomeIcon className="text-success text-xs" icon={faTrophy} />
            </div>,
            <span className="font-mono text-[9px] text-success border border-success/20 rounded px-1.5 py-0.5" style={{ background: 'rgba(74,222,128,0.07)' }}>+47 today</span>,
            '2.841',
          ...
          <div className="lg:col-span-3 bg-content1 border border-default-200 rounded-xl flex flex-col">
            ...
            <div className="p-5 space-y-4 flex-1">
              {MOCK_FOCUS_AREAS.map((area) => (
                <div
                  key={area.id}
                  className={`p-4 rounded-xl border ${focusBorderColor(area.color)} bg-content1`}
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right shrink-0 ml-4">
                      <p className={`font-bold text-lg leading-none ${scoreTextColor(area.accuracy)}`}>{area.accuracy}%</p>
                  ...
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`font-mono text-[9px] ${scoreTextColor(area.accuracy)}`}>{area.accuracy} / 100</span>
                    </div>
                    <div className="h-1.5 bg-default-200 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${focusBarColor(area.color)}`} style={{ width: `${area.accuracy}%` }} />
                    </div>
                  </div>
            ...
            <div className="bg-content1 border border-default-200 rounded-xl flex flex-col flex-1">
              ...
              <div className="divide-y divide-default-100">
                {MOCK_SESSIONS.map((session) => (
                  <div key={session.id} className="flex items-center gap-3 px-5 py-3 hover:bg-default-50 transition-colors cursor-pointer">
            ...
            {MOCK_DOMAINS.map((domain) => (
              <div key={domain.name} className="flex items-center gap-4">
                ...
                <div className="w-10 text-right shrink-0">
                  <span className={`font-mono font-semibold text-xs ${scoreTextColor(domain.score)}`}>{domain.score}%</span>
                </div>
        ...
        <footer className="flex items-center justify-between py-2 pb-4">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 border border-primary/30 rounded flex items-center justify-center" style={{ background: 'rgba(79,70,229,0.07)' }}>
