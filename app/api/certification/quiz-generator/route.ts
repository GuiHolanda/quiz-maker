...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (dbCert) {
      ...
      const fallback = INITIAL_CERTIFICATIONS_STATE.certifications.find((c) => c.label === certificationTitle);

      if (fallback) topics = fallback.topics;
    }

    if (!topics || topics.length === 0) {
      return NextResponse.json({ message: `Certification not found: ${certificationTitle}` }, { status: 400 });
    }
    ...
    const perTopicResults = await Promise.all(
      Array.from(allocation.entries()).map(async ([topicName, count]) => {
        const questions = await service.fetchStoredQuestions(certificationTitle, [topicName], count, session.user.id);
    ...
    if (shortfalls.length > 0) {
      ...
      return NextResponse.json({ message: `Insufficient questions for topics: ${details}` }, { status: 400 });
    }
