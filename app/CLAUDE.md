...
- `app/(workspace)/simulados/components/SimuladosListTab.tsx` — "sem simulados" com `onPress` para a aba "New" da página unificada
...
Página unificada que agrega **simulados de certificações e de concursos** em uma única rota `/simulados`. Monta todos os providers necessários e expõe duas abas: lista e criação.

| Arquivo | Papel |
|---|---|
| `page.tsx` | Monta `CertificationsProvider` + `PublicExamsProvider` + `CertSimuladosProvider` + `MockExamsProvider`; renderiza `SimuladosPageContent` com tabs List/New |
| `components/SimuladosListTab.tsx` | Lista unificada de cert simulados + mock exams com filtros (busca, fonte, tipo, quantidade, status); chips de status semanticamente distintos (`pending` → `default`, `in_progress` → `warning`, `answered` → `success`); botão de trash icon-only para deleção; modal de confirmação de deleção; modal de histórico de tentativas com navegação para resultado |
| `components/NewSimuladoTab.tsx` | Type picker (Certificação / Concurso) com ícone + título + descrição; oculto para planos sem acesso a concursos (renderiza `NewCertSimuladoForm` direto); `data-testid="type-option-{certification\|concurso}"` para E2E |
| `components/NewCertSimuladoForm.tsx` | Form de criação de cert simulado: Select cert + inputs nome/total + tabela de distribuição por tópico com validação de disponibilidade e botão "Gerar questões" para tópicos insuficientes |
| `components/NewMockExamForm.tsx` | Form de criação de mock exam: PublicExamManager + inputs nome/total + tabela de distribuição por matéria com validação de disponibilidade e botão "Gerar questões" |

#### Cards da lista (`SimuladosListTab`)

- **`data-testid="simulado-card"`** em cada card — usado pelos testes E2E para escopar ações por simulado específico
- **Status chips**: `pending` → `color="default"`, `in_progress` → `color="warning"`, `answered` → `color="success"`
- **Ação "Iniciar"**: `ensureCert|MockExamAnswers` é chamado fire-and-forget antes de `start*Attempt`; enquanto carrega, o card ativo recebe `border-primary` e os demais ficam `opacity-40`
- **Deleção**: trash icon-only no canto direito → modal de confirmação → remove do estado via `cert.removeSimulado` ou `mock.removeMockExam`
- **Histórico**: botão "Ver resultados" (quando `attemptCount > 0`) → modal com lista de tentativas e scores; "Ver detalhes" navega para a página de resultado da tentativa

#### Distribuição de questões

As tabelas de distribuição em `NewCertSimuladoForm` e `NewMockExamForm` mostram para cada tópico/matéria:
- Nome do tópico + subtítulo "N disponíveis" (abaixo do nome, em `text-default-400`)
- Chip de erro com `border border-danger bg-danger/5 rounded-lg` quando `questionCount > available` (substituiu o antigo `border-l-2 border-l-danger`)
- Botão "Gerar questões faltantes" que pré-seleciona cert/tópico no localStorage e navega para `/questions?tab=generate`
- Input numérico + botão remover
