...
| **Peso do domínio** | Média de `minQuestions` e `maxQuestions` (inteiros 0–100) |
...
| RN-02 | Peso do domínio = (`minQuestions` + `maxQuestions`) ÷ 2. Se todos os pesos forem 0, os domínios pesam igual | `computeExamReadiness` |
| RN-03 | O acerto de um domínio usa só as **30 respostas mais recentes** do domínio, vindas de tentativas **finalizadas e não expiradas** (`timedOut = false`). Resposta cuja questão não pertence a um domínio do edital é ignorada | Query do service + `computeExamReadiness` |
| RN-04 | Meta do domínio = `totalQuestions` distribuído pelos pesos com o método do maior resto (`distributeByWeight`) | `computeExamReadiness` |
...
- `ExamService.getExams` ganha uma terceira consulta em paralelo: `mockExamAttemptAnswer.findMany` filtrada por
  `attempt: { userId, finishedAt: { not: null }, timedOut: false }` e pelas questões dos exames listados. Status
  (`draft`/`active`/`completed`) não mudou; `deriveReadinessAndStatus` virou `deriveStatus`.
- `DashboardService` não ganha consulta: reaproveita `sectionAnswers`, que passa a trazer `examId` e `sectionId` da
  questão, e descarta as respostas de tentativas expiradas antes do cálculo. A consulta de exames passa a trazer
  `totalQuestions`, `passingScore` e o peso das seções.
...
| `shared/components/ui/ProgressTrack.tsx` | Nova prop `markerPercent` para o marcador da nota de corte |
| `dashboard/components/exams/ExamProgressRow.tsx` | Mesmas fases, em versão compacta |
...
| `building_bank` | `18/40 questões` | banco coberto ÷ meta, `bg-primary` | "Gere mais 22 questões…" + **Gerar questões** |
...
| Arquivo | Cobre |
|---|---|
| `tests/unit/lib/exam-readiness.test.ts` | RN-01 a RN-06 e o comparador (RN-09) |
| `tests/unit/api/services/exam.service.test.ts` | RN-01, RN-03 (filtro da query) e RN-05 via `getExams` |
| `tests/unit/api/services/dashboard.service.test.ts` | RN-10 e o descarte de respostas expiradas |
...
| `tests/unit/features/examReadinessNote.test.ts` | RN-08, incluindo as chaves de singular |
| `tests/e2e/tests/exams-list-unified.spec.ts` | RN-08 no navegador: sem simulado, o card mostra o banco e o link de gerar questões; o painel lista um domínio por linha |
