        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          ...
          <div className="col-span-2 md:col-span-1">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-6 h-6 rounded flex items-center justify-center" style={{ background: 'rgba(0,212,255,0.1)', border: '1px solid rgba(0,212,255,0.3)' }}>
            ...
            <p className="font-sans text-xs text-navy-500 leading-relaxed mb-4 max-w-52">
              {t('footer.description')}
            </p>
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-green-400" />
              <span className="font-mono text-xs text-green-400">{t('footer.operational')}</span>
            </div>
          </div>
            ...
            <ul className="space-y-2.5">
              {PLATFORM_LINKS.map((item) => (
                <li key={item.labelKey}>
                  <NextLink className="font-sans text-xs text-navy-400 hover:text-white transition-colors duration-200" href={item.href}>
                    {t(item.labelKey)}
                  </NextLink>
                </li>
              ))}
