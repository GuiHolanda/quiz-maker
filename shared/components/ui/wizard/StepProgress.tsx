...
export function StepProgress({ currentStep, namespace }: StepProgressProps) {
  const { t } = useTranslation();
  const percent = Math.round((currentStep / 3) * 100);
  const title = t(`${namespace}.step${currentStep}Title`);
    ...
    <div className="flex flex-col gap-2">
      <div className="flex justify-between items-end">
        <span className="text-sm font-semibold text-foreground">
          {t(`${namespace}.stepProgress`, { current: String(currentStep), total: '3', title })}
        </span>
