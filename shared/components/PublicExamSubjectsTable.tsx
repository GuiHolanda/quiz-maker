...
export interface PublicExamSubjectsTableHandle {
  startAdd: () => void;
}
...
export const PublicExamSubjectsTable = forwardRef<PublicExamSubjectsTableHandle, PublicExamSubjectsTableProps>(
  function PublicExamSubjectsTable(
    {
      selectedPublicExam,
      subjectsList,
      editable = false,
    ...
    useImperativeHandle(ref, () => ({ startAdd: () => setIsAddingSubject(true) }));
          ...
          <table className="w-full border-collapse">
            <thead className="bg-content2">
              <tr>
                ...
                <th className={TH}>{t('concurso.minQuestionsHeader')}</th>
                <th className={TH}>{t('concurso.maxQuestionsHeader')}</th>
            ...
            <tbody>
              {subjects.map((subject, index) => (
                <SubjectRow
                  ...
                  onTopicRemoved={
                    onTopicRemoved && subject.id ? (topicId) => onTopicRemoved(subject.id!, topicId) : undefined
                  }
                  onUpdate={onSubjectUpdated ? (newName, min, max) => handleUpdate(subject, newName, min, max) : undefined}
                />
              ))}
              {isAddingSubject && (
                <AddSubjectRow onAdd={handleAddSubject} onCancel={() => setIsAddingSubject(false)} />
              )}
            </tbody>
          </table>
        </div>
